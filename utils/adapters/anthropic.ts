...
function buildAnthropicBody(openaiReq: any, actualModel: any, isStream: any) {
  ...
  for (const msg of messages) {
    ...
    if (msg.role === "tool") {
      ...
      if (
        ...
        prev.content.some((b: any) => b.type === "tool_result")
      ) {
  ...
  if (openaiReq.tool_choice) {
    if (typeof openaiReq.tool_choice === "string") {
      ...
      body.tool_choice = { type: choiceMap[openaiReq.tool_choice as keyof typeof choiceMap] || "auto" };
 ...
 */
function toToolResultContent(content: any) {
  if (typeof content === "string") return content;
  if (Array.isArray(content)) {
    return content
      .map((b: any) => (b.type === "text" ? { type: "text", text: b.text } : b))
      .filter(Boolean);
  }
 ...
 */
function getStreamState(ctx: any) {
...
function anthropicUsageToOpenAI(usage: any) {
