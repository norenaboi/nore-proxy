...
export function calculateCost(model: any, inputTokens: any, outputTokens: any, cacheWriteTokens: any, cacheReadTokens: any, tokenAccountingVersion = null, ) {
  return calculateModelCost(
    ...
    cacheWriteTokens,
    cacheReadTokens,
    tokenAccountingVersion ?? undefined,
  );
}
...
export function logRequestEnd(
  ...
  tokenAccountingVersion: string | null = null,
  routingMetadata: RoutingMetadata | null = null,
) {
  ...
  const allowedRoutingMetadata: Record<string, unknown> = {};
  if (routingMetadata && typeof routingMetadata === "object") {
    const routing = routingMetadata as RoutingMetadata;
    for (const key of [
      "requested_model",
      "auto_model",
      "target_model",
