...
class KeyStateManager {
  ...
  _ensureRow(endpointKey: string, tokenHash: string, token: string): void {
   ...
   */
  _isRowUsable(row: KeyStateRow | undefined, endpointKey: string, tokenHash: string): boolean {
    if (!row) return true;
   ...
   */
  getUsableTokens(endpointKey: string, tokens: string[], { excludeHashes = new Set<string>() }: { excludeHashes?: Set<string> } = {}): Array<{ token: string; tokenHash: string }> {
    if (!Array.isArray(tokens) || tokens.length === 0) return [];

    const rows: KeyStateRow[] = this.stmtGetEndpointRows.all(endpointKey);
    const byHash = new Map(rows.map((r: KeyStateRow) => [r.token_hash, r]));

    const usable = [];
    for (const token of tokens) {
      const tokenHash = this.hashToken(endpointKey, token);
      if (excludeHashes.has(tokenHash)) continue;
      const row: any = byHash.get(tokenHash);
  ...
  recordSuccess(endpointKey: string, token: string): void {
    const tokenHash = this.hashToken(endpointKey, token);
   ...
   */
  recordFailure(endpointKey: string, token: string, statusCode: unknown, { sideline = true }: { sideline?: boolean } = {}): { tokenHash: string } {
    const tokenHash = this.hashToken(endpointKey, token);
    const now = this._now();
    this._ensureRow(endpointKey, tokenHash, token);
   ...
   */
  disableKey(endpointKey: string, token: string): { tokenHash: string } {
    const tokenHash = this.hashToken(endpointKey, token);
    this._ensureRow(endpointKey, tokenHash, token);
    this.stmtSetStatus.run({
      endpoint_key: endpointKey,
   ...
   */
  resetKey(endpointKey: string, { tokenHash = null, all = false }: { tokenHash?: string | null; all?: boolean } = {}): number {
    if (all) {
   ...
   */
  resetStats(endpointKey: string, { tokenHash = null, all = false }: { tokenHash?: string | null; all?: boolean } = {}): void {
   ...
   */
  getStatesForEndpoint(endpointKey: string, tokens: string[]): Array<KeyStateView & Record<string, unknown>> {
    const safeTokens = Array.isArray(tokens) ? tokens : [];

    const rows: KeyStateRow[] = this.stmtGetEndpointRows.all(endpointKey);
    const byHash = new Map(rows.map((r: KeyStateRow) => [r.token_hash, r]));

    const codeRows: Array<{ token_hash: string; status_code: number; count: number }> = this.stmtCodesForEndpoint.all(endpointKey);
    ...
    const now = this._now();
    return safeTokens.map((token: string, index: number) => {
      const tokenHash = this.hashToken(endpointKey, token);
      const row: any = byHash.get(tokenHash);
   ...
   */
  _reasonUnavailable(state: KeyStateView): string | null {
   ...
   */
  describeExhaustion(endpointKey: string): { message: string; details: string[] } {
   ...
   */
  buildExhaustionError(endpointKey?: string): Error {
    ...
    if (details.length) {
      (error as any).responseBody = { keyStates: details };
    }
    return error;
  }
