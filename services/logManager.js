...
function recordedCostMask(costs) {
...
export class LogManager {
  ...
  ensureRequestLogSchema() {
    ...
    const existing = new Set(this.db.prepare("PRAGMA table_info(request_logs)").all().map((column) => column.name));
    const definitions = {
    ...
    const migrate = this.db.transaction(() => {
      for (const column of columns) {
        if (!existing.has(column)) {
          this.db.exec(`ALTER TABLE request_logs ADD COLUMN ${column} ${definitions[column]}`);
          migrated = true;
        }
      }
      this.db.exec(`
    ...
    migrate();
    const rollupColumns = new Set(this.db.prepare("PRAGMA table_info(request_daily_rollups)").all().map((column) => column.name));
  ...
  requestProjectionValues(logEntry, row = {}) {
  ...
  backfillRequestProjections() {
    ...
    const runBatch = this.db.transaction((rows) => rows.forEach((row) =>
  ...
  getRequestAggregates(filters = {}) { return this.aggregateRequests(filters); }
  getRequestRangeAggregates(ranges = []) { return ranges.map((range) => ({ ...range, ...this.aggregateRequests(range) })); }
  getBulkApiKeyAggregates(filters = {}) {
    return this.getAggregateRows(filters, "key");
  }
  getBulkModelAggregates(filters = {}) {
    return this.getAggregateRows(filters, "model");
  }
  getBulkDashboardAggregates(ranges = []) {
  ...
  getAggregateRows(filters = {}, groupBy = null) {
  ...
  getApiKeyAggregates(filters = {}) { return this.getBulkApiKeyAggregates(filters); }
  getModelAggregates(filters = {}) { return this.getBulkModelAggregates(filters); }
  ...
  getCostBreakdown(filters = {}) {
    ...
    return this.db.prepare(`SELECT request_model AS model, token_accounting_version AS tokenAccountingVersion, ${input} AS inputTokens, ${output} AS outputTokens, ${cacheWrite} AS cacheWriteTokens, ${cacheRead} AS cacheReadTokens FROM ${source} WHERE ${clauses.join(" AND ")} GROUP BY request_model, token_accounting_version`).all(params).map((row) => ({ model: row.model || "Unknown", tokenAccountingVersion: row.tokenAccountingVersion || null, inputTokens: Number(row.inputTokens) || 0, outputTokens: Number(row.outputTokens) || 0, cacheWriteTokens: Number(row.cacheWriteTokens) || 0, cacheReadTokens: Number(row.cacheReadTokens) || 0 }));
  }

  getRecentRequests(limit = 50, filters = {}) { return this.getRequestHistory({ ...filters, limit: Math.min(Math.max(Number(limit) || 50, 1), 200) }); }
  ...
  getRequestHistoryFilters() {
    const models = this.db.prepare(`SELECT DISTINCT request_model AS value FROM request_logs
      WHERE type = 'request_end' AND projection_version >= 2 AND request_model IS NOT NULL AND request_model != '' ORDER BY value`).all().map((row) => row.value);
    const apiKeys = this.db.prepare(`SELECT api_key_id, api_key_masked, key_name FROM request_logs
      ...
      GROUP BY api_key_id, api_key_masked, key_name ORDER BY key_name, api_key_masked`).all().map((row) => ({
