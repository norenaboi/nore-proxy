...
type CancelablePromise<T = void> = Promise<T> & { cancel?: (error?: Error) => void };
type RoutingAttemptRunner = (state: any, endpoint: any, keyAttempt?: number) => Promise<any>;
type Usage = { prompt_tokens?: number; completion_tokens?: number; prompt_tokens_details?: Record<string, number> };
...
function anthropicToOpenAIMessages(anthropicReq: any) {
  const messages: any[] = [];
...
function anthropicToolChoiceToOpenAI(choice: any) {
...
function toolResultText(content: any) {
...
function persistUpstreamError({
  ...
  const context = (buildUpstreamErrorContext as any)({
    modelName,
    endpointInfo,
    requestHeaders,
    upstreamUrl,
...
function withAttemptContext(error: Error, endpointInfo: any, headers: DynamicRecord = {}, fullUrl: string | null = null, responseBody?: any): Error {
...
async function executeMessagesRouting(requestId: string, requestedModel: string, runAttempt: RoutingAttemptRunner): Promise<any> {
...
function anthropicErrorType(statusCode: any) {
...
function anthropicErrorEnvelope(error: any, statusCode: any) {
...
router.post("/v1/messages", verifyApiKey, async (req: any, res: any) => {
...
async function makeMessagesRequest(requestId: string, anthropicReq: DynamicRecord, modelName: string, apiKey: string, openaiMessages: any[]): Promise<any> {
  return executeMessagesRouting(requestId, modelName, (state: any, endpointInfo: any) =>
    makeMessagesAttempt(
      requestId,
      anthropicReq,
      modelName,
...
async function makeMessagesAttempt(
  requestId: string,
  baseAnthropicReq: DynamicRecord,
  modelName: string,
  apiKey: string,
  baseOpenaiMessages: any[],
  ...
  try {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (apiFormat === "anthropic") {
        ...
        Object.keys(openaiReq).forEach((k: any) => {
...
async function streamMessages(req: Request, res: Response, requestId: string, anthropicReq: DynamicRecord, modelName: string, apiKey: string, openaiMessages: any[]): Promise<any> {
  const abortController = new AbortController();
  let activeStream: StreamWithDestroy | null = null;
  let cancelActiveAttempt: ((error?: Error) => void) | null = null;
  ...
  try {
    return await executeMessagesRouting(requestId, modelName, (state: any, endpointInfo: any) =>
      streamMessagesAttempt(
        ...
        abortController,
        () => clientAborted,
        (stream: any) => { activeStream = stream; },
        (cancel: any) => { cancelActiveAttempt = cancel; },
      ),
    );
  } finally {
...
async function streamMessagesAttempt(
  ...
  setAttemptCancel: (cancel: ((error?: Error) => void) | undefined) => void,
) {
  const anthropicReq = clone(baseAnthropicReq);
  const openaiMessages = clone(baseOpenaiMessages);
  if (endpointOverride?.generationDefaults) {
  ...
  try {
    ...
    const streamPromise = (consume as any)(...args) as CancelablePromise;
...
function streamAnthropicPassthrough(
  ...
  promise.cancel = (error: any) => cancel?.(error);
  return promise;
}
...
function streamOpenAIToAnthropic(
  ...
  const cleanup = () => {
    ...
    stream.off("error", onError);
    stream.off("close", onClose);
    delete (res as any).__routingState;
  };
  const settle = (error?: Error) => {
    if (settled) return;
    settled = true;
