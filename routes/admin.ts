 ...
 */
function computeCostsFromLogs(logs: any) {
...
router.get("/api/requests", verifySession, (req: any, res: any) => {
  ...
  const status = (req.query.status as QueryValue)?.trim() || null;
...
function buildStoredModel(definition: DynamicRecord, existing: DynamicRecord = {}) {
...
router.delete("/api/models", verifySession, async (req: any, res: any) => {
  try {
    const name = typeof req.body.name === "string" ? req.body.name.trim() : "";
    if (!name) return res.status(400).json({ error: "Model name required" });
    const modelsPath = getModelsPath();
...
router.patch("/api/models/visibility", verifySession, async (req: any, res: any) => {
  try {
    const name = typeof req.body.name === "string" ? req.body.name.trim() : "";
    if (!name) return res.status(400).json({ error: "Model name required" });
    const modelsPath = getModelsPath();
...
router.patch("/api/models/toggle", verifySession, async (req: any, res: any) => {
  try {
    const name = typeof req.body.name === "string" ? req.body.name.trim() : "";
    if (!name) return res.status(400).json({ error: "Model name required" });
    const modelsPath = getModelsPath();
...
router.get("/api/endpoints", verifySession, async (req: any, res: any) => {
  try {
    ...
    const data = JSON.parse(content) as JsonDocument;
    ...
    for (const [key, endpoint] of Object.entries(data)) {
      ...
      const maskedTokens = rawTokens.map((t: any) =>
        t.length > 8
...
router.post("/api/endpoints", verifySession, async (req: any, res: any) => {
  try {
    ...
    if (Array.isArray(req.body.tokens)) {
      tokens = req.body.tokens.map((t: any) => String(t).trim()).filter(Boolean);
    ...
    let generationDefaults: any = validateGenerationDefaults(req.body.generationDefaults);
...
router.put("/api/endpoints", verifySession, async (req: any, res: any) => {
  try {
    ...
    if (Array.isArray(req.body.tokens)) {
      incomingTokens = req.body.tokens.map((t: any) => String(t).trim()).filter(Boolean);
...
router.delete("/api/endpoints", verifySession, async (req: any, res: any) => {
  try {
    ...
    const modelsContent = fs.existsSync(modelsPath)
      ...
      : null;
    const modelsData: DynamicRecord = modelsContent === null
      ? { models: {} }
...
router.get("/api/endpoints/:version/models", verifySession, async (req: any, res: any) => {
  try {
...
function readEndpointTokens(version: any) {
  const endpointsPath = getEndpointsPath();
...
router.post("/api/endpoints/:version/keys/disable", verifySession, (req: any, res: any) => {
  try {
    ...
    const token = tokens.find(
      (t: any) => keyStateManager.hashToken(endpointKey, t) === tokenHash,
    );
    if (!token) {
  ...
  try {
    ...
    if (updates.autoModelMaxTargetAttempts !== undefined) {
      ...
      const blockers = Object.entries(models)
        .filter(([, config]) =>
          modelType(config as DynamicRecord) === "auto" &&
          (config as DynamicRecord).disabled !== true &&
          Number.isInteger((config as DynamicRecord).maxTargetAttempts) &&
          (config as DynamicRecord).maxTargetAttempts > updates.autoModelMaxTargetAttempts,
        )
        .map(([name]) => name);
      if (blockers.length) {
        return res.status(409).json({
 ...
 */
function validateGenerationDefaults(input: any) {
  const defaults: Record<string, { enabled: boolean; value: number | null }> = {
...
function validateKeyRotation(input: any) {
...
function validateKeyHealth(input: any) {
