...
- Every persisted `endpoints.json` change must refresh the in-memory endpoint configuration with the established reload path. Endpoint key rotation keeps no cross-request state, so a reload carries no rotation position to reset.
...
- An endpoint's `apiFormat` belongs to one of three categories, and the category is the modality of every model served by that endpoint. Nothing else sets a model's modality; the admin UI displays it read-only. Each category has its own client route, and a model registers the matching `type` (`chat`, `image`, `embedding`) for clients to discover.
  ...
  - `image` — `openai-images`, `openai-images-generations`, `gemini-interactions`. `openrouter-images` is the former name of `openai-images`; `normalizeApiFormat()` maps it on load and lookup, and the admin API rejects it.
...
- The `openai-embeddings` and `openai-images` adapters forward unknown top-level params untouched. That passthrough is what makes an arbitrary OpenAI-compatible provider work without new code; do not replace either with an allow-list. `openai-images` posts to `/v1/images` and `openai-images-generations` to `/v1/images/generations`; the generations module re-exports the `openai-images` adapter. `gemini-embeddings` and `gemini-interactions` need real translation, since Google's native surfaces are not OpenAI-shaped. An endpoint outside the matching category is refused up front rather than posted to.
